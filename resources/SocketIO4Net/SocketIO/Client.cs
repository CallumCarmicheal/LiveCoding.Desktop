...
namespace SocketIOClient {
    ...
    public class Client : IDisposable, SocketIOClient.IClient {
        ...
        public Client(string url, string path)
            : this(url, path, WebSocketVersion.Rfc6455) {
        }
        public Client(string url, string path, NameValueCollection headers) 
            : this(url, path, WebSocketVersion.Rfc6455, headers) {
        }
        public Client(string url, string path, WebSocketVersion socketVersion)
            : this(url, path, WebSocketVersion.Rfc6455, null) {
        }
        public Client(string url, string path, WebSocketVersion socketVersion, NameValueCollection headers) {
        ...
        public string generateURL(string QUERY) {
            ...
            string
                Host = uri.Host,
                Port = uri.Port + "",
                tmpPath = (Path == "" || Path == null) ? "/socket.io" : "/" + Path,
                tmpQuery = (QUERY == "") ? "" : "/" + QUERY,
                URL = wsScheme + "://" + Host + ":" + Port + tmpPath + tmpQuery;

            //return schema + "://" + this.hostname + port + this.path + query
             
            URL = wsScheme + "://www.livecoding.tv:443/live.eio/" + tmpQuery;

            System.Diagnostics.Debug.WriteLine("GENERATED URL: " + URL);
            Console.WriteLine("GENERATED URL: " + URL);
            return "https://www.livecoding.tv:443/live.eio/?EIO=3&transport=polling&t=1447875532171-0";
        }
                ...
                if (!(this.ReadyState == WebSocketState.Connecting || this.ReadyState == WebSocketState.Open)) {
                    try {
                        ...
                        if (string.IsNullOrWhiteSpace(this.HandShake.SID) || this.HandShake.HadError) {
                            this.LastErrorMessage = string.Format("Error initializing handshake with {0}:{1}/{2}", generateURL(uri.Query));
                            this.OnErrorEvent(this, new ErrorEventArgs(this.LastErrorMessage, new Exception()));
                        } else {
                            string wsScheme = (uri.Scheme == Uri.UriSchemeHttps ? "wss" : "ws");
                            this.wsClient = new WebSocket(
                                generateURL(this.HandShake.SID),
                                string.Empty,
                                this.socketVersion);
        ...
        protected void ReConnect() {
            ...
            else {	// we didn't connect - try again until exhausted
        ...
        protected void closeWebSocketClient() {
            if (this.wsClient != null) {
                ...
                if (this.wsClient.State == WebSocketState.Connecting || this.wsClient.State == WebSocketState.Open) {
                    try { this.wsClient.Close(); } catch { Trace.WriteLine("exception raised trying to close websocket: can safely ignore, socket is being closed"); }
                }
                this.wsClient = null;
            }
        }
        ...
        private void wsClient_OpenEvent(object sender, EventArgs e) {
            ...
            if (this.Opened != null) {
                try { this.Opened(this, EventArgs.Empty); } catch (Exception ex) { Trace.WriteLine(ex); }
            }
        ...
        protected void OnErrorEvent(object sender, ErrorEventArgs e) {
            ...
            if (this.Error != null) {
                try { this.Error.Invoke(this, e); } catch { }
            }
        ...
        protected void OnSocketConnectionClosedEvent(object sender, EventArgs e) {
            if (this.SocketConnectionClosed != null) {
                try { this.SocketConnectionClosed(sender, e); } catch { }
            }
        ...
        protected void OnConnectionRetryAttemptEvent(object sender, EventArgs e) {
            if (this.ConnectionRetryAttempt != null) {
                try { this.ConnectionRetryAttempt(sender, e); } catch (Exception ex) { Trace.WriteLine(ex); }
            }
        ...
        protected void requestHandshake(Uri uri) {
            ...
            using (WebClient client = new WebClient()) {
                try {
                    ...
                    string host = generateURL(uri.Query);
                    string oldHost = string.Format("{0}://{1}:{2}/socket.io/1/{3}", uri.Scheme, uri.Host, uri.Port, uri.Query);
                    value = client.DownloadString(host); // #5 tkiley: The uri.Query is available in socket.io's handshakeData object during authorization
